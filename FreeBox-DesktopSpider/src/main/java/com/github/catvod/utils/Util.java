 ...
 *
 * 与安卓版的差异：
 * - 移除了所有安卓 UI / 系统服务相关方法（copy / loadUrl / addView / removeView / loadWebView），
 *   这些方法依赖 ClipboardManager、WebView、Activity 等桌面端不存在的安卓运行时；
 *   如某站点蜘蛛需要此类能力，应在该站点类内改用桌面等价实现或跳过。
 * - fixUrl / webHeaders 改用 java.net.URI。
 * - base64 编解码沿用 android.util.Base64（由本工程提供的桌面 shim 实现）。
 */
public class Util {
